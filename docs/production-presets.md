...
The connected cases are `friends-front-back`, explicitly labeled **Friends · S04E01 · “For the record…” → “front and back”**, and `few-good-men-courtroom`, labeled ***A Few Good Men* · courtroom scene**. Both use the same catalog, preview, rehearsal, and recording-guide path. Their sources remain in `norbertachatelli`, while Uizador ships release copies under `web/prebuilt-guides/`. They remain HTML/SVG guides; no runtime scene generation, external scene hosting, or video conversion is required.
